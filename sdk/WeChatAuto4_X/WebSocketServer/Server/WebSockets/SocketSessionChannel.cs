        ...
        _MonitorTask = Task.Run(async () =>
        {
            ...
            try
            {
                await foreach (var message in this.channel.Reader.ReadAllAsync(cts.Token))
                {
                    ...
                    {
                        ServerOperationLog.Expired(message, "queue_wait");
                        try
                        {
                            await message.handler!.SendAsync(new RequestData
                            {
                                Type = "error",
                                RequestId = message.RequestId,
                                Data = "Command expired before execution.",
                            });
                        }
                        catch (Exception ex)
                        {
                            logger.LogDebug(ex, "Unable to return expired command response {RequestId}", message.RequestId);
                        }
                        continue;
                    }
