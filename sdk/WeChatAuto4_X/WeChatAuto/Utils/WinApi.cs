...
namespace WeChatAuto.Utils
{
    public class WinApi
    {
        ...
        public static bool GetWindowVisibility(
            IntPtr hWnd,
            out bool isVisible,
        ...
        {
            if (!GetWindowVisibility(hWnd, out var isVisible, out var isMinimized))
                return false;

            if (!isVisible)
                ShowWindow(hWnd, 5); // SW_SHOW; tray-hidden windows are not necessarily minimized.
            if (isMinimized)
                ShowWindow(hWnd, 9); // SW_RESTORE

            return GetWindowVisibility(hWnd, out isVisible, out isMinimized)
                && isVisible
                && !isMinimized;
        }
