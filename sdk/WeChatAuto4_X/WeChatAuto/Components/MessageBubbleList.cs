...
namespace WeChatAuto.Components
{
    ...
    public class MessageBubbleList
    {
        ...
        {
            var semanticRetry = Retry.WhileNull(() =>
            {
                var root = this._Client.MainWindow.FindFirstDescendant(cf =>
                    cf.ByControlType(ControlType.List)
                        .And(cf.ByClassName("mmui::RecyclerListView"))
                        .And(cf.ByAutomationId("chat_message_list")));
                root ??= this._Client.MainWindow.FindFirstDescendant(cf =>
                    cf.ByControlType(ControlType.List)
                        .And(cf.ByClassName("mmui::RecyclerListView"))
                        .And(cf.ByName("消息")));
                return root;
            }, TimeSpan.FromSeconds(1), TimeSpan.FromMilliseconds(100));
            if (semanticRetry.Success)
                return semanticRetry.Result.AsListBox();
