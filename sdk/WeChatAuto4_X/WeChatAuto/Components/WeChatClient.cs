...
namespace WeChatAuto.Components
{
    ...
    public partial class WeChatClient : IDisposable
    {
        ...
        /// <summary>
        /// Restores a minimized or tray-hidden WeChat main window without
        /// activating it when it is already visible.
        /// </summary>
        public bool EnsureMainWindowVisible(string reason = "sdk_operation")
        {
            var stopwatch = Stopwatch.StartNew();
            var handle = (IntPtr)GetHandler();
            var stateKnown = WinApi.GetWindowVisibility(handle, out var wasVisible, out var wasMinimized);
            var restored = WinApi.EnsureWindowVisible(handle);
            stopwatch.Stop();
            if (!stateKnown || !wasVisible || wasMinimized || !restored)
            {
                _logger.Trace(
                    $"WeChat window visibility check reason={reason}, process_id={ClientProcessId}, " +
                    $"handle={handle}, prior_visible={wasVisible}, prior_minimized={wasMinimized}, " +
                    $"success={restored}, duration_ms={stopwatch.Elapsed.TotalMilliseconds:F3}");
            }
            return restored;
        }
        ...
        public async Task SendMessage(string who, string message, OneOf<string, string[], List<string>> atUser = default, ChatRefer refer = null, CancellationToken cancellationToken = default)
            => await ChatContent.SendMessage(who, message, atUser, refer, cancellationToken);
        /// <summary>
        ...
        public async Task SendMessage(string message, OneOf<string, string[], List<string>> atUser = default, ChatRefer refer = null, CancellationToken cancellationToken = default)
            => await ChatContent.SendMessage(message, atUser, refer, cancellationToken);
        ...
        /// <summary>扫描全部表情分类并生成本地目录。</summary>
        public async Task<StickerCatalogScanResult> ScanAllStickers() => await ChatContent.ScanAllStickers();

        /// <summary>发送已扫描目录中的表情。</summary>
        public async Task SendSticker(string who, string category, string nameOrHash)
            => await ChatContent.SendSticker(who, category, nameOrHash);
