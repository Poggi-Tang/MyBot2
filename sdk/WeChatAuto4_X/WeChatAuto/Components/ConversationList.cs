...
namespace WeChatAuto.Components
{
    ...
    public class ConversationList
    {
        ...
        internal bool SearchWhoCore(UIA3Automation automation, string who)
        {
            ...
            // WeChat 4.x exposes stale Selection/SelectionItem state for the
            // conversation list. Use the search field's highlighted result
            // and verify that the chat header actually changed.
            return _SearchFromKeyboard(automation, who);
        }
        /// <summary>
        /// 打开who指定的子窗口
        /// </summary>
        ...
        private bool _SearchLiveConversations(string who, ListBox root)
        {
            ...
            freshTarget.AsListBoxItem().Select();
        ...
        //如果会话列表不存在who,再查询
        private bool _SearchFromKeyboard(UIA3Automation automation, string who)
        {
            if (string.IsNullOrWhiteSpace(who))
                return false;

            var edit = _Client.MainWindow.FindFirstDescendant(cf =>
            ...
            var popupResult = Retry.WhileNull(
                () => _Client.MainWindow.FindFirstDescendant(cf =>
                    cf.ByControlType(ControlType.List)
                        .And(cf.ByAutomationId("search_list"))),
            ...
            {
                Thread.Sleep(100);
                var title = _Client.ChatContent.ChatHeader.GetTitleCore(automation)?.Title;
                if (string.Equals(title?.Trim(), who.Trim(), StringComparison.Ordinal))
                    return true;
            }
        ...
        internal SimpleConversation GetConversationItemFromName(string name)
        {
            ...
            var metadata = new HashSet<string> { "消息免打扰", "已置顶" };
            var contentLines = new List<string>();
            foreach (var rawLine in aryName.Skip(1))
            {
                var line = rawLine.Trim();
                if (string.IsNullOrEmpty(line) || metadata.Contains(line) || Regex.IsMatch(line, @"^\[\d+条\]$"))
                    continue;
                if (Regex.IsMatch(line, @"^(?:(?:昨天|前天)\s+\d{1,2}:\d{2}|\d{1,2}:\d{2}|昨天|前天|星期[一二三四五六日天]|\d{1,2}/\d{1,2})$"))
                {
            ...
            conversation.ConversationContent = string.Join("\n", contentLines);
        ...
        {
            const string path = @"/Group/Custom/Group/Group/Group/Custom/Custom/Group/Group/Group/Group/Group/Group/List[@Name='会话'][@AutomationId='session_list']";
            ...
            throw new InvalidOperationException("无法恢复微信会话列表，已取消本次操作以避免占死 UI 调度线程。");
        }
        //往上滚动
